...
const DocxPreviewHeader: React.FC<DocxPreviewHeaderProps> = ({
  hasMarkdown,
  previewMode,
  onPreviewModeChange,
  downloadDisabled,
  ...
  const downloadTitle = previewMode === "markdown" ? "下载 Markdown" : "下载 Word";
    ...
    <header className="flex h-[60px] shrink-0 items-center justify-between gap-2 border-b border-(--ant-color-border-secondary) bg-(--ant-color-bg-container) px-3">
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <Typography.Text type="secondary">
          {previewMode === "markdown" ? "Markdown 预览" : "Word 预览"}
        </Typography.Text>
      </div>
...
export default React.memo(DocxPreviewHeader);
