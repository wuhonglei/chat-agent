...
from .utils import (
    ...
    format_weather_alerts,
    format_weather_daily_forecast,
    format_weather_hourly_forecast,
    make_request,
)
...
async def search_city(
    ...
    """
    搜索城市信息（仅支持城市级查询，不支持国家/地区名称）。
    适用于：查询天气、获取城市 LocationID。location 参数必须是城市名（如"北京""Nairobi"），
    不能是国家名（如"中国""肯尼亚"）或行政区划名。
    时区/时间相关问题请使用联网搜索工具，不要使用天气工具。
    @return:
    ...
    try:
        ...
        return ToolResult(
            structured_content=city_search_response,
            content=format_cities(city_search_response.location),
        )
    except Exception:
        raise
...
async def get_current_weather(
    ...
    try:
        ...
        return ToolResult(
            structured_content=weather_now_response,
            content=format_current_weather(weather_now_response.now),
        )
    except Exception:
        raise
...
async def get_weather_hourly_forecast(
    ...
    hours: Literal["24h", "72h", "168h"] = Field(
        default="24h",
        description="预报小时数，可选值：24h（24小时预报）、72h（72小时预报）、168h（168小时预报）",
    ),
    ...
    try:
        ...
        return ToolResult(
            structured_content=weather_hourly_response,
            content=format_weather_hourly_forecast(weather_hourly_response.hourly),
        )
    except Exception:
        raise
...
async def get_weather_daily_forecast(
    ...
    try:
        ...
        return ToolResult(
            structured_content=weather_daily_response,
            content=format_weather_daily_forecast(weather_daily_response.daily),
        )
...
async def get_weather_alerts(
    ...
    try:
        ...
        return ToolResult(
            structured_content=weather_alert_response,
            content=format_weather_alerts(weather_alert_response.warning),
        )
    except Exception:
        raise
