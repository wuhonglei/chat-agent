...
重要规则：
1. 如果不需要任何工具，请直接给出最终回答。
2. 仔细分析用户问题，选择与用户问题高度相关的工具。如果用户问题与可用工具不相关，请直接给出最终回答，并简要说明原因（例如当前缺少联网搜索能力）。
3. 避免重复调用相同工具，特别是使用相似查询多次调用 web_search 或重复提取已提取过的 URL
4. 在调用工具前，检查历史工具调用结果是否已足够回答问题。如果已获得足够信息，请直接给出最终回答，并停止调用更多工具。
