...
- [`app/services/chat_upload/__init__.py`](app/services/chat_upload/__init__.py) **无需改动后处理：当前未 re-export 上述符号
...
| [`app/services/chat/kb_rag_context_service.py`](app/services/chat/kb_rag_context_service.py) | SQL `file_id` → `content_id`；`RetrievedChunk.file_id` → `content_id`；`candidate_file_ids` → `candidate_content_ids` |
| [`app/utils/multimodal.py`](app/utils/multimodal.py) | `collect_attachment_file_ids` → `collect_attachment_content_ids` |
| [`app/services/chat/chat_orchestrator.py`](app/services/chat/chat_orchestrator.py) | 调用处同步重命名 |
| [`app/schemas/chat.py`](app/schemas/chat.py) | `KbContextBlock.id` 的 `description="附件 file_id"` → `"附件 content_id"` |
...
[`collect_attachment_content_ids`](app/utils/multimodal.py) 从 message blocks 提取：
...
**v2 的 `messages.content_blocks` 已包含 `storage_key` 与 `storage_version`**（见 [`tests/message/user_message.json`](tests/message/user_message.json) 及上传服务返回值）。迁移**不依赖** `attachment_files` 表查路径。
...
| 文件 | 函数 | 变更 |
|------|------|------|
| [`attachment.py`](app/services/chat_upload/attachment.py) | `save_chat_attachment` | `conversation_id: str`（必填）；移除 `db` 挂载逻辑 |
| [`image.py`](app/services/chat_upload/image.py) | `save_chat_image` | `conversation_id: str`；删除 `mount_conversation_attachment` / `upsert_attachment_file` 调用 |
| [`markdown.py`](app/services/chat_upload/markdown.py) | `save_chat_markdown` | 同上 |
| [`pdf.py`](app/services/chat_upload/pdf.py) | `save_chat_pdf` | 同上；derived 路径用 **分配后** pdf display_name 的 stem |
| [`file.py`](app/api/file.py) | `upload_chat_attachment` | `Form(...)` 必填 |
...
| 文件 | 变更 |
|------|------|
| [`tests/test_multimodal_kb_rag.py`](tests/test_multimodal_kb_rag.py) | `collect_attachment_file_ids` → `collect_attachment_content_ids`；`file_ids` 变量名 |
| [`tests/test_kb_rag_context_service.py`](tests/test_kb_rag_context_service.py) | `RetrievedChunk(file_id=...)` → `content_id`；`candidate_file_ids` → `candidate_content_ids` |
| [`tests/test_chat_orchestrator_kb_context.py`](tests/test_chat_orchestrator_kb_context.py) | 断言 `candidate_content_ids` |
| [`tests/test_pdf_markdown_flow.py`](tests/test_pdf_markdown_flow.py) | 路径 mock 改为会话级目录 |
