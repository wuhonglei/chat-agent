...
- **存储（采用方案 B）**  
  - 单独建表 **`user_context`**，按 **`user_id` + `conversation_id`** 联合存储（唯一键或主键），便于以后跨会话复用、按用户迁移、以及同一用户多对话独立上下文）。  
  - 表结构示例：`user_id`、`conversation_id`、`facts`（JSON 数组或文本）、`preferences`（JSON 数组或文本）、可选 `summary_before_window` / `recent_summary`、`updated_at` 等。
...
- **生成时机**  
  - **异步归纳**：在 [ChatService.stream_message](app/services/chat_service.py) 流程结束、助手消息落库后，触发一次「归纳本轮+历史事实/偏好」的 LLM 调用（或队列任务），写入或更新 **user_context 表**中对应 **user_id + conversation_id** 的记录。  
  - **使用时机**：下次同一会话请求时，根据 **user_id + conversation_id** 查询 user_context，在 [get_system_prompt_for_response_generation](app/prompts/prompt_utils.py) 或 MCP 的 system 中注入「当前已知用户事实与偏好」片段（若存在）。

- **实现注意**  
  - 归纳 prompt 要明确「仅输出可验证事实与明确偏好、不要编造」。  
  - 可与「超出窗口的摘要」共用同一套 long-term context 逻辑（见下）。
...
- **流程**：  
  ...
  2. 在组装发给 LLM 的 `messages` 前做 **token 预算**：预留 system、user_context 注入、当前用户消息、当前轮工具消息（MCP/组件）、以及为回复预留的 token；用 [TokenCalculator.count_messages_tokens](app/utils/token.py) 对历史消息从**新到旧**累加，超出预算的**前面**（更早的）消息不再加入本次请求，得到本次使用的 **history_messages**。  
  3. 可选：把「被截掉」的旧消息交给异步任务做摘要，写入 **user_context** 表的 `summary_before_window` / `recent_summary`，下次请求时在 system 中注入，使窗口外的信息以压缩形式存在。
...
- **落点**：  
  ...
  - 摘要写入（可选）：在 [MessageService.update_assistant_message](app/services/message_service.py) 之后或 ChatService 流式收尾处，若有被截断的旧消息，可调用 `context_summary_service` 归纳并更新 **user_context** 表（user_id + conversation_id）。  
...
- **写入 user_context**：表为 **user_context**（user_id + conversation_id 唯一）。将摘要写入 **`summary_before_window`** 或 **`recent_summary`**（前者表示「窗口外更早的摘要」，后者可为「最近一次截断产生的摘要」）；若该行不存在则 insert，存在则 update 对应字段及 `updated_at`。可与「用户事实/偏好」的异步归纳**合并为同一次任务**：同一请求下先写 facts/preferences，再写本摘要，或统一在一个 service 内顺序执行，减少对 user_context 的多次写。
...
| **ChatMessageItem / MessageDb** | 已有 `tool_calls`、`component_tool_calls`、`message_metadata`；需补充 **tool result 存储**（以便最近 1 轮完整展开）及 **`tool_calls_summary`** 字段（存更早消息、token≥2K 时的结构化摘要）；事实/偏好存 **user_context** 表（user_id + conversation_id）。 |

---

## 四、建议实施顺序

1. **用户事实与偏好**：新建 **user_context** 表（主键或唯一键 user_id + conversation_id）+ 归纳逻辑 + 按 user_id + conversation_id 查询并注入 system（可先做「仅事实」或纯摘要，再细化事实/偏好结构）。  
...
3. **历史由后端处理 + token 级滑动窗口（策略 B）**：后端按 conversation_id 拉取会话消息；实现 `truncate_history_by_tokens`（或等价），在组装 messages 前按 token 预算从新到旧截断得到 history_messages；配置 `max_history_tokens`。可选：被截断的旧消息异步摘要写入 user_context，并在 system 中注入。
...
- [ ] **user_context 表**：设计并新建表 `user_context`，主键或唯一键 `(user_id, conversation_id)`，字段含 `user_id`、`conversation_id`、`facts`（JSON/文本）、`preferences`（JSON/文本）、`summary_before_window`、`recent_summary`、`updated_at`；编写 Alembic 迁移并执行。
...
- [ ] **截断接入**：在 chat API 内、调用 ChatService.stream_message 前（或 ChatService/Agent 入口处），先拉取会话消息，再执行「先轮数后 token」截断，将 `history_messages` 传入 ChatService；在 token 预算中预留 system、user_context、当前用户消息、当前轮工具消息、回复预留。

### 3. 用户事实与偏好

- [ ] **归纳逻辑**：实现 `context_summary_service`（或等价）中的「用户事实/偏好」归纳：输入本轮+历史消息（或仅窗口内+user_context 现有内容），调用 LLM 产出结构化 facts / preferences；Prompt 明确「仅输出可验证事实与明确偏好、不要编造」。
- [ ] **写入 user_context**：在 ChatService 流式结束、助手消息落库后，**异步**触发归纳并按 `user_id` + `conversation_id` 写入或更新 user_context 表（insert or update）；若与「窗口外摘要」共用任务，可在同一异步流程内顺序执行。
- [ ] **注入 system**：在 [get_system_prompt_for_response_generation](app/prompts/prompt_utils.py) 或 MCP system 组装处，根据 `user_id` + `conversation_id` 查询 user_context，若存在 facts/preferences 则格式化为片段拼入 system；请求链路需能取得 `user_id`（如从 JWT 或 conversation 关联）。
...
- [ ] **写入 user_context**：将摘要写入 user_context 的 `summary_before_window` 或 `recent_summary`；若该行不存在则 insert，否则 update 对应字段与 `updated_at`；可与「用户事实/偏好」写入合并为同一次异步任务。
...
- [ ] **端到端**：从「按 conversation_id 拉取 → 轮数+token 截断 → stream_message(history_messages) → 窗口内工具消息按方案 3 组装」全链路联调；验证 user_context 与窗口外摘要的读写及 system 注入。
