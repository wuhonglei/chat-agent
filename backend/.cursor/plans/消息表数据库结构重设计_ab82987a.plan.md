...
- **执行顺序**：在 `[chat_service.py](app/services/chat/chat_service.py)` 中硬编码为：MCP → Component → Response Generation
...
| 模块     | 文件                                          | 变更要点                                                                                      |
| ------ | ------------------------------------------- | ----------------------------------------------------------------------------------------- |
| 模型     | `app/models/message_db.py`                  | 新增 content_parts、tool_call_steps；删除 content，废弃 tool_calls 等                               |
| Schema | `app/schemas/content.py`                    | ContentPart（新建）                                                                           |
| Schema | `app/schemas/chat.py`                       | ChatRequest、ChatMessageItem、CollectedResponse（引用 ContentPart）                             |
| Schema | `app/schemas/tool_call.py`                  | ToolCallStep（新建）                                                                          |
| 消息服务   | `app/services/message/message_db.py`        | 读写新字段、create_user_message 支持 content_parts                                                |
| 对话服务   | `app/services/chat/chat_service.py`         | 按 tool_call_steps 驱动执行、Collect 聚合                                                         |
| Agent  | `mcp_tools_agent`、`component_tools_agent`   | 接口保持，由 ChatService 按 step 调用                                                              |
| 工具     | `app/utils/message.py`                      | format_chat_message_for_llm 支持 content_parts                                              |
| API    | `app/api/chat.py`、`app/api/conversation.py` | Chat 接收 content_parts；新增 `POST /api/conversations/{id}/uploads` 及 `GET .../artifacts/...` |
| 历史     | `app/utils/history_truncate.py`             | token 计算考虑 content_parts                                                                  |
...
1. **Phase 1**：工具调用重构（tool_call_steps）
  ...
  - 兼容期双写（`tool_call_steps` 与 `tool_calls` / `component_tool_calls` 等并存）
  - 验证后移除旧字段（与 Phase 3 删除废弃列一并执行）
