...
| **ChatMessageItem / MessageDb / ToolResultMessage** | 已有 `tool_calls`、`component_tool_calls`、`message_metadata`；需补充 **tool result 存储**（含 **summary** 字段，与 [ToolResultMessage.summary](app/schemas/tool_call.py) 对应）以便最近 1 轮完整展开及更早消息注入摘要。事实/偏好存 **user_profile** 表（user_id）；会话级摘要存 **user_context** 表（user_id + conversation_id）。 |
