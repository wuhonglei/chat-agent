---
name: 聊天上下文压缩方案（最终版）
overview: 基于当前 chat_service 的三阶段流程，落实用户事实/偏好、历史处理与窗口内工具消息策略；历史来源采用前端主导，消息摘要存放在 ToolCallResultMessage.summary。
todos:
  - id: todo-1770133254585-k2jauy112
    content: ""
    status: pending
isProject: false
---
...
| **消息摘要存储** | 窗口内更早、token≥2K 的工具消息摘要存入 MessageDb 的 `**tool_calls_summary**` 字段（assistant 消息上）。 | **ToolCallResultMessage.summary**：每条工具结果摘要存放在对应 **ToolCallResultMessage** 的 **summary** 字段；若持久化，则 DB 中 tool result 记录含 **summary** 字段，不再使用 assistant 消息上的 `tool_calls_summary`。 |
...
- **b. 其他轮**：工具调用统一使用 **summary** 参与组装：
  - 若该条 **ToolCallResultMessage.summary** 已有内容，则**直接使用**该 summary（不传完整 content）。
  - 若 **summary 无内容**，则看 **content**：
    - content 未超出 2K token：**直接将 content 作为 summary 使用**（即本轮组装时等价于用 content，但不改持久化里的 summary 字段，仅本次使用）。
    - content 超出 2K token：对 content **进行截断**（或规则/模板生成简短摘要），用截断/摘要结果作为本次注入内容；可选写回 **ToolCallResultMessage.summary** 或 DB 中 tool result 的 summary 字段供后续复用。
...
**结论（已采用）**：**方案 3（最近 1 轮完整 tool 消息）+ 更早消息按 token 分流**。最近 1 轮展开为完整 assistant（tool_calls）+ tool（result）；更早消息中 token < 2K 原样保留，token ≥ 2K 则用规则/模板生成简短结构化摘要并存入 **ToolCallResultMessage.summary**，组装时仅注入该摘要。摘要不必用 LLM，以规则/模板为主。
...
2. **窗口内其他轮**：工具调用统一使用 **ToolCallResultMessage.summary** 参与组装（详见上文「history_messages 处理流程」）：
  - **summary 有内容**：直接使用该 summary，不传完整 content。
  - **summary 无内容**：若 **content** 未超出 2K token，则**直接将 content 作为 summary 使用**（仅本次注入，可不写回持久化）；若 content 超出 2K token，则对 content **进行截断**（或规则/模板生成简短摘要），用截断/摘要结果作为本次注入内容，可选写回 **ToolCallResultMessage.summary** 或 DB tool result 的 summary 字段供后续复用。
...
- **存储**：摘要存放在 **ToolCallResultMessage.summary**（内存/流式结构）；若历史持久化到 DB，则每条 tool result 对应一条记录，该记录带 **summary** 字段。当某条历史 tool result content 超出 2K 且无 summary 时，可截断或生成摘要并写回 summary（可在 process_history_messages 内同步完成，或异步落库时写回）。
- **组装逻辑**：在 process_history_messages 内：① 识别「最近 1 轮」的助手消息（含 tool_calls）→ 保留完整 assistant + tool 消息；② 对更早的含 tool_calls 的消息，若 ToolCallResultMessage 有 **summary** 则仅保留/注入该 summary，若无则按 content 是否超出 2K 决定「直接用 content 作为 summary」或「截断/摘要后注入」。
- **阈值**：2K token 可配置（CompressionConfig 中 `tool_message_summary_threshold_tokens = 2000`），便于调参。

**LLM 摘要**：不必作为默认。简短结构化摘要以**规则/模板**为主（工具名 + 「得到 N 条结果」或关键 result 片段）。若后续语义不足，再对部分长 result 引入 LLM 摘要，且建议**异步落库时**完成并写入 **ToolCallResultMessage.summary**（及 DB tool result 的 summary 字段）。
...
| **ChatMessageItem / MessageDb / ToolCallResultMessage** | 已有 `tool_calls`、`component_tool_calls`、`message_metadata`；需补充 **tool result 存储**（含 **summary** 字段，与 [ToolCallResultMessage.summary](app/schemas/llm.py) 对应）以便最近 1 轮完整展开及更早消息注入摘要。事实/偏好存 **user_profile** 表（user_id）；会话级摘要存 **user_context** 表（user_id + conversation_id）。 |
...
2. **滑动窗口内工具调用**：在 [process_history_messages](app/services/chat_service.py) 内实现——最近 1 轮完整 tool 消息；其他轮用 **summary**（有则直接使用，无则 content 未超 2K 时直接将 content 作为 summary 使用，超 2K 时截断后注入，可选写回 **ToolCallResultMessage.summary**）；补充 MessageDb 的 tool result 存储（含 summary）；窗口外忽略工具调用并对内容做摘要写入 user_context。
...
这样在不改变现有流式协议与 API 的前提下，完成「记录事实与偏好」「前端主导历史 + 后端 token 级截断」「窗口内工具消息：最近 1 轮完整 + 更早按 token 分流并用 ToolCallResultMessage.summary 存储摘要」的上下文压缩方案，并与现有 [chat_service](app/services/chat_service.py) 流程和配置兼容。
...
- **MessageDb / tool result 存储**：增加 **tool result 存储**（使每条 assistant 的 tool_calls 能对应到 tool 结果内容），且每条 tool result 记录含 **summary** 字段（与 ToolCallResultMessage.summary 对应）；若当前未存，编写 Alembic 迁移并执行。
...
### 2. 历史由前端主导 + 轮数与 token 截断

- **按 history_ids 拉取历史**：在 chat API 流程中保持按前端传入的 **history_ids** 拉取消息（如 [message_service.get_history_messages_by_ids](app/services/message_service.py)）；不改为按 conversation_id 全量拉取。API 保留 `removed_message_ids`，`history_ids` 为必传或约定必传。
...
- **截断接入**：在 chat API 内、调用 ChatService.stream_message 前（或 ChatService/Agent 入口处），先按 history_ids 拉取消息，再执行「先轮数后 token」截断，将 `history_messages` 传入 ChatService；在 token 预算中预留 system、user_profile + user_context 注入、当前用户消息、当前轮工具消息、回复预留。
...
### 5. history_messages 处理流程（process_history_messages）

- **落点**：在 [ChatService.process_history_messages](app/services/chat_service.py) 内实现对 history_messages 的完整处理；在「按 history_ids 拉取 → stream_message」之间调用，保证 MCP / 响应生成使用的已是处理后的历史。
- **滑动窗口内的工具调用**：① 近 1 轮使用**完整工具调用**（assistant + tool result 原样保留）；② 其他轮使用 **summary**：若 ToolCallResultMessage.summary 有内容则直接使用，若无则判断 content 是否超出 2K——未超出则**直接将 content 作为 summary 使用**，超出则**对 content 进行截断**（或规则/模板生成摘要）后注入，可选写回 summary。
- **滑动窗口外的工具调用**：① **忽略**窗口外的工具调用（不进入本次发给 LLM 的 messages）；② 对窗口外**整段内容**做摘要，写入 **user_context**，在 system 中注入（摘要管道见上一节）。
- **MCP 与 Response 共用**：process_history_messages 输出后的 history_messages 被 MCP 与 Response 共用，保证两处对历史工具消息的处理一致。

### 6. 滑动窗口内历史工具调用（方案 3 + summary/content/截断，摘要存 ToolCallResultMessage.summary）

- **最近 1 轮完整展开**：在 process_history_messages 内识别「最近 1 轮」的含 tool_calls 的助手消息，保留完整 assistant（tool_calls）+ tool（result）；依赖 MessageDb 与拉取逻辑已提供 tool result。
- **其他轮：用 summary，无则 content 或截断**：若 ToolCallResultMessage 已有 **summary** 则只注入该 summary；若无则 content 未超出 2K 时**直接将 content 作为 summary 使用**，超出 2K 时**截断**（或规则/模板生成摘要）后注入，可选写回 **ToolCallResultMessage.summary** 或 DB tool result 的 summary 字段。
- **摘要/截断规则**：规则/模板生成单条 tool result 的 summary（工具名 + 「得到 N 条结果」或关键 result 片段），无 LLM；可选后续为长 result 增加异步 LLM 摘要并写回 **ToolCallResultMessage.summary**。

### 7. 联调与配置

- **端到端**：从「前端传 history_ids → 按 history_ids 拉取 → 轮数+token 截断 → stream_message(history_messages) → 窗口内工具消息按方案 3 组装（摘要用 ToolCallResultMessage.summary）」全链路联调；验证 **user_profile**（事实/偏好）与 **user_context**（会话摘要）的读写及 system 注入；验证新会话能复用 user_profile 中的事实与偏好。
