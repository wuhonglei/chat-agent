...
| 测试场景 | QPS | P50 | P95 | P99 | Connect | 说明 |
|----------|-----|-----|-----|-----|---------|------|
| 本地 gunicorn 直连（8 workers） | **2,289** | 40ms | 68ms | 113ms | - | 服务器本地，基准 |
| 本地 gunicorn 直连（1 worker） | **1,304** | 75ms | - | 93ms | - | 单 worker 对比 |
| 本地 nginx 代理（8 workers） | **1,703** | 56ms | 92ms | 114ms | - | 服务器本地，排除网络 |
| 直接访问业务应用（HTTP） | **217** | 70ms | 2074ms | 4074ms | 374ms | 绕过公共服务 |
| 完整链路（HTTPS） | **113** | 577ms | 1382ms | 2234ms | 485ms | 包含公共服务 nginx + TLS |
...
nginx 代理 (8 workers): 1,703 QPS
    ↓ -87.3% (网络延迟)
直接访问业务应用 (HTTP): 217 QPS
    ↓ -47.9% (公共服务 nginx + TLS)
完整链路 (HTTPS): 113 QPS
```

**公共服务 nginx + TLS 带来约 48% 的性能损失**。

### 4.4 公共服务 nginx 性能损失

| 指标 | 直接访问 (HTTP) | 完整链路 (HTTPS) | 损失 |
|------|-----------------|------------------|------|
| QPS | 217 | 113 | **-48%** |
| P50 | 70ms | 577ms | +724% |
| P99 | 4074ms | 2234ms | -45% |
| Connect | 374ms | 485ms | +30% |

公共服务开销包括：
1. **TLS 握手** — Connect 485ms vs 374ms（+111ms）
2. **nginx 代理开销** — 反向代理、header 处理、请求转发
3. **HTTPS 加解密** — CHACHA20-POLY1305 计算开销

### 4.5 Workers 数量对比（1 vs 8）
