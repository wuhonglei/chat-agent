...
| 内网直连业务应用（公共服务器→10.0.24.3:3000） | **2,002** | 46ms | 130ms | 2ms | HTTP，绕过公共服务 |
| 公共服务 nginx + TLS（公共服务器本地） | **311** | 312ms | 415ms | 175ms | HTTPS，含 TLS 握手 |
| 直接访问业务应用（深圳→业务应用） | **217** | 70ms | 4074ms | 374ms | HTTP，绕过公共服务 |
...
内网直连业务应用 (公共服务器→10.0.24.3:3000): 2,002 QPS
    ↓ -84.5% (公共服务 nginx + TLS)
公共服务 nginx + TLS (公共服务器本地): 311 QPS
    ↓ -63.7% (外网延迟)
完整链路 (深圳→公共服务→业务应用): 113 QPS
```

**公共服务 nginx + TLS 带来 84.5% 的性能损失**，是最大的性能瓶颈。

### 4.4 公共服务 nginx + TLS 性能损失
