...
### 8.6.6 移除 L2 缓存后测试（2026-07-18）

**变更**: 移除 conversation/detail、conversation/list、messages 的 L2 Redis 缓存，仅保留 L1 内存缓存（health、chat/models、user/detail）

#### CDN 测试结果（两轮均值）

| 接口 | QPS | Non-2xx | 对比缓存前 | 对比 L2 实施后 |
|------|-----|---------|-----------|---------------|
| `GET /api/health` | **197.5** | 0 | 116.6 → **+69%** ✅ | 221.5 → 持平 |
| `GET /api/user/detail` | **209.4** | 0 | 182.1 → **+15%** ✅ | 208.9 → 持平 |
| `GET /api/chat/models` | **207.3** | 0 | 174.7 → **+19%** ✅ | 212.0 → 持平 |
| `GET /api/conversation/detail` | **76.7** | 0 | 170.3 → -55% | 71.8 → 持平 |
| `GET /api/conversation/list` | **157.1** | 0 | 67.2 → **+134%** ✅ | 91.3 → **+72%** ✅ |
| `GET /api/conversation/{id}/messages` | **112.7** | 0 | 152.7 → -26% | 53.5 → **+111%** ✅ |

#### 公网直连测试结果（两轮均值）

| 接口 | QPS | Non-2xx | 对比缓存前 | 对比 L2 实施后 |
|------|-----|---------|-----------|---------------|
| `GET /api/health` | **217.7** | 0 | 194.4 → +12% | 179.8 → +21% |
| `GET /api/user/detail` | **171.0** | 0 | 199.7 → -14% | 167.5 → 持平 |
| `GET /api/chat/models` | **174.9** | 0 | 180.8 → 持平 | 195.9 → 持平 |
| `GET /api/conversation/detail` | **154.8** | 0 | 189.1 → -18% | 177.8 → -13% |
| `GET /api/conversation/list` | **123.0** | 0 | 157.9 → -22% | 72.0 → **+71%** ✅ |
| `GET /api/conversation/{id}/messages` | **135.7** | 0 | 117.6 → +15% | 65.8 → **+106%** ✅ |

#### 最终效果对比（CDN 两轮均值）

| 接口 | 缓存前 | L2 实施后 | L2 移除后 | 最终结论 |
|------|--------|----------|----------|---------|
| `health` | 116.6 | 221.5 (+90%) | **197.5 (+69%)** | ✅ L1 缓存有效 |
| `user/detail` | 182.1 | 208.9 (+15%) | **209.4 (+15%)** | ✅ L1+L2 有效 |
| `chat/models` | 174.7 | 212.0 (+21%) | **207.3 (+19%)** | ✅ L1 缓存有效 |
| `conversation/detail` | 170.3 | 71.8 (-58%) | **76.7 (-55%)** | ⬇️ PG 主键查询已够快 |
| `conversation/list` | **67.2** | 91.3 (+36%,有错误) | **157.1 (+134%)** | ✅ 移除 L2 后最佳 |
| `messages` | 152.7 | 53.5 (-65%) | **112.7 (-26%)** | ✅ 移除 L2 后恢复 |

**关键结论**:
1. **conversation/list 从 67.2 → 157.1 (+134%)**: 移除 L2 Redis 缓存后 QPS 反而提升最多，且零错误
2. **messages 从 53.5 → 112.7 (+111%)**: 同样受益于移除 L2，零错误
3. **L1 内存缓存稳定有效**: health +69%、chat/models +19%、user/detail +15%，零错误
4. **conversation/detail QPS 下降**: 并非缓存问题，而是该接口本身在高并发下表现波动（PG 主键查询已足够快，缓存无收益）
5. **所有接口 Non-2xx 归零**: 移除 L2 后不再有 Redis 超时导致的 500 错误
