...
- 删除会话：`DELETE /api/conversation/delete/{conversation_id}`（先 purge 该会话已发布站点：摘 `current`、删除 `data/sites/{slug}/`、删 `published_sites` 行，再删 conversation；否则 FK 会失败且公网 URL 成孤儿站）
...
检索闸门、治理字段与管理页 API 见 `backend/docs/用户管理.md`。
...
可选 `language`（BCP 47）：前端在 `streamMessage` 里传 `navigator.language || "zh-CN"`。`ChatSessionAgent` 写入 system prompt：优先用用户 query 的语言作答；**无法从 query 推断时**才用该值。续流 `POST /api/chat/stream/resume` **不带**该字段（续的是已建好的会话，不再重建 system）。空值不注入这段指令。
...
- `language` 只影响当轮 system 回退语言，不会改写用户消息或历史；不要把它当成强制输出语言。
- 删除会话会同步下线并删除该会话的静态站快照；只删 `user_data/conversations/{id}` 不会摘 `data/sites/{slug}/current`。
