...
按标题与消息正文（`content_blocks` ILIKE）搜索当前用户会话；仅匹配
`status=done` 的消息；同一会话去重，标题命中优先于正文。
...
`match_type` 为 `title` | `user` | `assistant`；正文命中时 `snippet` 为片段，标题命中可为空。
...
- `message_metadata`：请求配置、回复关系等元数据；Agent 触达轮次上限时还会写入 `iteration_checkpoint`（`iterations_used` / `continue_budget`）
...
- 会话搜索是标题 + 正文 ILIKE，不是向量语义搜索；空 `q` 会被校验拒绝。
