...
[`backend/app/services/base_service/__init__.py`](backend/app/services/base_service/__init__.py) 将目录定义为「基础设施服务」（外部系统、DB、存储、Embedding、LLM）。但实际还包含大量**业务域**代码：
...
[`backend/app/api/file.py`](backend/app/api/file.py) 中：
...
- API：[`backend/app/api/file.py`](backend/app/api/file.py)
- 工具：[`backend/app/utils/multimodal.py`](backend/app/utils/multimodal.py)（`user_upload_file_path`）
- 对话 RAG：[`backend/app/services/chat/kb_rag_context_service.py`](backend/app/services/chat/kb_rag_context_service.py)（`get_user_upload_dir`）
- 测试：[`backend/tests/test_pdf_markdown_flow.py`](backend/tests/test_pdf_markdown_flow.py)（mock 路径需同步）
...
新建独立包，与 [`AGENTS.md`](backend/AGENTS.md) 中「按领域分包」一致，且避免与 [`backend/app/utils/file.py`](backend/app/utils/file.py) 在命名上混淆：

**推荐包名：`backend/app/services/chat_upload/`**（表示「对话场景下的上传与处理」，不包含头像 COS 流程）
...
**路径常量：** `chat_attachment_service` 里 `_BACKEND_ROOT = Path(__file__).resolve().parents[3]` 在迁入 `backend/app/services/chat_upload/attachment.py` 后，**目录深度不变**（仍为 backend 根），一般无需改计算方式；迁包后做一次本地路径冒烟（上传 + 预览）即可确认。
...
| B（更贴领域） | 将 `upload_avatar` 迁到例如 `backend/app/services/user/avatar_service.py`，`FileService` 类可删除或变薄；`base_service` 只暴露 `ObjectStorageService` | 用户域更清晰 |
...
1. **新建** `backend/app/services/chat_upload/` 与 `__init__.py`（可显式 `__all__` 导出对外 API，便于其它层引用）。
2. **移动** 上述 5 个业务文件到新包，**批量修正** 包内互引与对外 import（`app.services.base_service.chat_*` → `app.services.chat_upload.*`）。
3. **更新** `api/file.py`、`multimodal.py`、`kb_rag_context_service.py` 及 `backend/tests/test_pdf_markdown_flow.py` 中的导入路径；测试中 monkeypatch 的模块字符串（如 `app.services.base_service.pdf_markdown_converter`）改为新路径。
4. **删除** `base_service` 下已迁走的旧文件（或在一版内保留薄 shim `from app.services.chat_upload.xxx import *` 以兼容外部 fork——若仓库无此类需求可直接删）。
5. **运行** `make lint`、`make test`（至少包含 `backend/tests/test_pdf_markdown_flow.py` 与 `backend/tests/test_multimodal_kb_rag.py` 若存在相关用例），必要时 `make check`。
...
- **命名冲突：** 避免新建 `backend/app/services/file/` 与 `backend/app/utils/file` 在口头沟通中混淆；`chat_upload` 更不易歧义。
