...
| 任务                       | 状态    | 说明                                                                                                                                                           |
| ------------------------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CompressionConfig 扩展     | ✅ 已完成 | 已增加 `max_history_tokens`、`max_history_rounds`、`tool_message_summary_threshold_tokens`（[backend/app/schemas/config.py](backend/app/schemas/config.py)）                        |
| 历史截断（轮数 + token）         | ✅ 已完成 | [backend/app/utils/history_truncate.py](backend/app/utils/history_truncate.py)：`truncate_history_by_rounds_and_tokens`、`count_chat_message_tokens`、`truncate_text_to_tokens` |
| 截断接入 stream_response     | ✅ 已完成 | [backend/app/services/chat_service.py](backend/app/services/chat_service.py)：拉取后先截断再 `process_history_messages`                                                              |
...
- **历史来源（采用前端主导）**：**由前端决定本次上下文包含哪些历史消息**。前端传 `history_ids`（本次要使用的历史消息 ID 列表）；后端根据 `history_ids` 从 DB 拉取对应消息（如 [message_service.get_history_messages_by_ids](backend/app/services/message_service.py)），得到 `history_messages`，再在组装发给 LLM 的 messages 前可做 **token 级截断**（从新到旧累加，超出预算的更早消息不加入）。API 保留 `removed_message_ids` 用于删除/重试等；`history_ids` 为前端必传或约定必传。
- **历史如何进 LLM**：[BaseAgent._compose_messages](backend/app/agents/base.py) 用 [format_chat_message_for_llm](backend/app/utils/message.py) 把每条 `ChatMessageItem` 转成仅 `role` + `content`（以及可选的 `reasoning`），**历史中的 tool_calls / component_tool_calls 并未传给模型**，只传了助手文本内容。
- **现有压缩**：[ContextCompactor](backend/app/utils/context_compactor.py) 仅用于 MCP **工具返回的 markdown 结果**（按 query 相关性 + token 阈值裁剪），不涉及对话历史或用户画像。
...
对 **history_messages** 的统一处理建议放在 [ChatService.process_history_messages](backend/app/services/chat_service.py) 中完成，在「按 history_ids 拉取 → 传入 stream_message」之间调用，保证后续 MCP / 响应生成使用的已是处理后的历史。
...
- **异步归纳**：在 [ChatService.stream_message](backend/app/services/chat_service.py) 流程结束、助手消息落库后，触发一次「归纳本轮+历史事实/偏好」的 LLM 调用（或队列任务），写入或更新 **user_profile** 表中对应 **user_id** 的记录（merge 或 replace，按产品需求）。
- **使用时机**：任意会话（含新创建的）请求时，在 [get_system_prompt_for_response_generation](backend/app/prompts/prompt_utils.py) 或 MCP 的 system 组装处：① 按 **user_id** 查 **user_profile**，若存在 facts/preferences 则格式化为片段拼入 system；② 再按 **user_id + conversation_id** 查 **user_context** 注入本会话摘要（见下节）。
...
- **流程**：
  ...
  2. 在组装发给 LLM 的 `messages` 前做 **token 预算**：预留 system、user_profile 与 user_context 注入、当前用户消息、当前轮工具消息（MCP/组件）、以及为回复预留的 token；用 [TokenCalculator.count_messages_tokens](backend/app/utils/token.py) 对历史消息从**新到旧**累加，超出预算的**前面**（更早的）消息不再加入本次请求，得到本次实际使用的 **history_messages**。
...
- **落点**：
  ...
  - 摘要写入（可选）：在 [MessageService.update_assistant_message](backend/app/services/message_service.py) 之后或 ChatService 流式收尾处，若有被截断的旧消息，可调用 `context_summary_service` 归纳并更新 **user_context** 表（user_id + conversation_id，仅会话级摘要）。
  - 读 summary：在 system 组装处根据 **user_id + conversation_id** 读 **user_context** 并拼进 system。

**窗口外消息摘要管道（截断 → 摘要 → 写入 user_context → 下次注入 system）**

- **触发时机**：在完成「轮数 + token 截断」并得到本次 `history_messages` 后，若存在**被截掉的旧消息**（即按 history_ids 拉取的消息列表中去掉本次保留部分后剩余的消息），则触发本管道；可在 [MessageService.update_assistant_message](backend/app/services/message_service.py) 之后或 chat 流式收尾处调用，**异步执行**，不阻塞本次响应。
...
- **下次注入 system**：在组装 system 的环节（如 [get_system_prompt_for_response_generation](backend/app/prompts/prompt_utils.py) 的调用链、或 MCP system 组装处），根据 **user_id + conversation_id** 查询 **user_context**，若存在 `summary_before_window` / `recent_summary` 则取出；将该摘要作为固定格式的一段拼入 system，例如：「以下是本对话中较早轮次的摘要，供参考：\n{摘要内容}」。注意在 token 预算中预留该段长度（或对摘要本身做最大 token 限制）。至少要在**响应生成**的 system 中注入；若希望 MCP 工具决策也利用长期上下文，可在 MCP 的 system 中同样注入。
...
- **方案 3：窗口内最近 1～2 轮保留完整 tool 消息**：在 [BaseAgent._compose_messages](backend/app/agents/base.py) 中，对 `history_messages` 里**最近若干条**助手消息，若含 tool_calls，则将其展开为标准的 assistant（tool_calls）+ tool（result）消息再追加到 messages；需把 `ChatMessageItem` 中的 `tool_calls` 与 DB 中存储的 tool result 对应起来。更占 token，但信息最完整。

**采用方案（方案 3 + 更早消息按 summary / content / 截断）**：

1. **最近 1 轮**：保留**完整**工具消息。即：对滑动窗口内「最近一轮」含工具调用的助手消息，在组装 messages 时展开为标准的 assistant（tool_calls）+ tool（result），原样传给 LLM。需确保 DB 中已存有 tool result（与 tool_calls 对应）；若当前落库未存 result，需在 [MessageService](backend/app/services/message_service.py) / MessageDb 中补充存储并在拉取历史时一并返回。
...
- **落点**：上述「窗口内近 1 轮完整 + 其他轮用 summary / content / 截断」逻辑在 [ChatService.process_history_messages](backend/app/services/chat_service.py) 中实现；下游 [BaseAgent._compose_messages](backend/app/agents/base.py) 或共用 history 格式化处消费已处理后的 `history_messages`。
...
| **ChatMessageItem / MessageDb / ToolResultMessage** | 已有 `tool_calls`、`component_tool_calls`、`message_metadata`；需补充 **tool result 存储**（含 **summary** 字段，与 [ToolResultMessage.summary](backend/app/schemas/llm.py) 对应）以便最近 1 轮完整展开及更早消息注入摘要。事实/偏好存 **user_profile** 表（user_id）；会话级摘要存 **user_context** 表（user_id + conversation_id）。 |
...
2. **滑动窗口内工具调用**：在 [process_history_messages](backend/app/services/chat_service.py) 内实现——最近 1 轮完整 tool 消息；其他轮用 **summary**（有则直接使用，无则 content 未超 2K 时直接将 content 作为 summary 使用，超 2K 时截断后注入，可选写回 **ToolResultMessage.summary**）；补充 MessageDb 的 tool result 存储（含 summary）；窗口外忽略工具调用并对内容做摘要写入 user_context。
...
这样在不改变现有流式协议与 API 的前提下，完成「记录事实与偏好」「前端主导历史 + 后端 token 级截断」「窗口内工具消息：最近 1 轮完整 + 更早按 token 分流并用 ToolResultMessage.summary 存储摘要」的上下文压缩方案，并与现有 [chat_service](backend/app/services/chat_service.py) 流程和配置兼容。
...
- **CompressionConfig 扩展** ✅ 已完成：在 [backend/app/schemas/config.py](backend/app/schemas/config.py) 的 CompressionConfig 中已增加 `max_history_tokens`（默认 32000）、`max_history_rounds`（默认 5）、`tool_message_summary_threshold_tokens`（默认 2000）；[backend/app/core/config.py](backend/app/core/config.py) 已加载。待补充：`window_out_summary_enabled`、`summary_max_tokens`。
...
- **ChatMessageItem / 拉取历史**：若新增 tool result 存储（含 summary），同步更新 [backend/app/schemas/chat.py](backend/app/schemas/chat.py) 的 ChatMessageItem 及 [backend/app/services/conversation_service.py](backend/app/services/conversation_service.py) 或 message_service 的拉取逻辑，使历史消息包含 tool result 及其 **summary**。

### 2. 历史由前端主导 + 轮数与 token 截断 ✅ 已完成

- **按 history_ids 拉取历史**：已保持按 [message_service.get_history_messages_by_ids](backend/app/services/message_service.py) 拉取；API 保留 `removed_message_ids`、`history_ids`。
- **轮数 + token 截断**：已实现 [backend/app/utils/history_truncate.py](backend/app/utils/history_truncate.py) 中 `truncate_history_by_rounds_and_tokens(messages, max_rounds, max_tokens, token_calculator)`，返回 `(kept_messages, truncated_messages)`；含 `count_chat_message_tokens`、`truncate_text_to_tokens`。
- **截断接入**：已在 [ChatService.stream_response](backend/app/services/chat_service.py) 内、`get_history_messages_by_ids` 之后、`process_history_messages` 之前执行截断，将截断后的 `history_messages` 传入 `process_history_messages` 再 `stream_message`。
...
- **注入 system**：在 [get_system_prompt_for_response_generation](backend/app/prompts/prompt_utils.py) 或 MCP system 组装处：① 根据 **user_id** 查询 **user_profile**，若存在 facts/preferences 则格式化为片段拼入 system（新会话与旧会话均复用）；② 再根据 user_id + conversation_id 查询 user_context 注入本会话摘要（见下一节）。请求链路需能取得 `user_id`（如从 JWT 或 conversation 关联）。
...
- **落点**：已在 [ChatService.process_history_messages](backend/app/services/chat_service.py) 内实现；在「按 history_ids 拉取 → 截断 → stream_message」之间调用，MCP / 响应生成共用处理后的历史。
