...
- **依赖**：[backend/pyproject.toml](backend/pyproject.toml) 增加 `pgvector>=0.3.0`（或与当前环境兼容的版本）。
...
### 2. MessageDb 新增字段（[backend/app/models/message.py](backend/app/models/message.py)）
...
- **代码**：删除 [backend/app/models/user_profile_db.py](backend/app/models/user_profile_db.py)，从 [backend/app/models/**init**.py](backend/app/models/__init__.py) 移除 UserProfileDb。
...
- **模型**：与现有 [ContextCompactor](backend/app/utils/context_compactor.py) 一致，使用 `settings.embedding_model`（如 DashScopeEmbeddings）；写入时将当前模型名写入 `embedding_model`，检索时仅使用 `embedding_model` 与当前配置一致且 **deleted_at IS NULL** 的行。
...
- 在 [ChatService.stream_message](backend/app/services/chat/chat_service.py) **开头**（阶段1 MCP 工具调用之前）：
...
- [ResponseGenerationAgent.stream_execute](backend/app/agents/response_generation_agent.py) 调用时传入 `user_message=user_message`。
...
- [chat_service](backend/app/services/chat/chat_service.py) 的 `_run_window_out_and_profile_tasks` 中，将 `UserProfileService.upsert_user_profile(user_id, facts=..., preferences=...)` 改为 **UserProfileItemService.batch_upsert_items(user_id, facts, preferences)**，不再写 user_profiles。
