---
name: 会话文件存储迁移
overview: 本期聚焦聊天附件上传与预览路径迁移：统一 id/storage_key 语义，将附件上传目录迁移为 user_data/<user_id>/uploads/{raw,derived} 并建立会话挂载，同时回填 messages.content_blocks；workspace 目录迁移暂不纳入本期范围。
todos:
  ...
  - id: upgrade-upload-flow
    content: 改造上传链路为 uploads/raw 与 uploads/derived 分层存储 + conversation 挂载，并在响应中双写引用字段
    status: completed
...
- 本期迁移只覆盖聊天附件，不调整 workspace 物理目录：
  - **附件存储迁移**：从当前 `user_data/<user_id>/uploads` 扁平目录逐步迁移到 `user_data/<user_id>/uploads/{raw,derived}`；先统一附件块 `id` 语义，保持 64 位 SHA256 并兼容 `kb_file_chunk_embeddings.file_id`；新增 `storage_key` 承载 `raw/<sha>.pdf`、`raw/<sha>.jpg`、`derived/<sha>.md` 等相对 `uploads` 的存储路径。
...
flowchart LR
    ...
    idContract --> uploadsRaw[UploadsRawStore]
    idContract --> uploadsDerived[UploadsDerivedStore]
    idContract --> embeddingIndex[EmbeddingIndex]
    uploadsRaw --> fileIndex[FileIndex]
    uploadsDerived --> fileIndex
...
说明：
- **第一层：附件存储迁移**。附件块 `id` 是稳定业务标识，必须优先兼容现有 embedding/RAG；`storage_key` 是相对 `uploads` 的存储 key，用目录表达 raw/derived 语义。
...
### Phase 1：附件上传链路升级为“uploads 分层目录 + 会话挂载”

- `upload` 接口增加 `conversation_id`（先可选，灰度后改必填）。
- 写入策略：
  - 原始文件写入 `uploads/raw/<sha>.<ext>`
  - 衍生文件写入 `uploads/derived/<sha>.md`
...
- 结合消息 `conversation_id` 建挂载关系，并保证历史文件可通过新 `storage_key` 访问：
  - 推荐将旧扁平目录文件复制或硬链接到 `user_data/<user_id>/uploads/<storage_key>`。
...
- [backend/app/services/chat_upload/attachment.py](/Users/apple/Desktop/code/chat-agent/backend/app/services/chat_upload/attachment.py)
  - 抽离路径构建与引用生成逻辑，接入 uploads 分层目录与 resolver。
...
- 真实文件仍位于 `user_data/<user_id>/uploads/<storage_key>`；数据库只记录文件元数据、存储 key、legacy source 与挂载关系，不存放文件内容。
- 预览接口安全策略：
  - 旧 `/api/file/preview/{user_id}/{filename}` 在兼容期保留，但不扩大能力范围。
  - 新 `/api/file/preview/{user_id}/{storage_key:path}` 预览可先沿用现有无需登录的预览兼容策略，但服务端必须通过 resolver 限制 `storage_key` 只能落在该用户 `uploads/{raw,derived}` 或旧 `uploads` 扁平文件允许范围内。
  - 若后续启用强权限模式，新 `id/storage_key` 预览必须登录，并校验 `conversation.user_id == auth.user_id` 与挂载关系。

## 会话挂载与数据库索引约定

- 物理存储路径：
  - `user_data/<user_id>/uploads/<storage_key>`
...
- 附件文件实体表：`attachment_files`
  ...
  - 兼容字段：`legacy_source`，用于历史文件尚未迁入 `uploads/{raw,derived}` 分层目录时回退旧 `uploads` 扁平路径。
...
- Resolver 处理流程：
  ...
  4. 优先将 `storage_key` 映射到 `uploads/<storage_key>`；若文件未迁入分层目录且 `legacy_source` 存在，则回退旧 `uploads` 扁平路径。
...
- `storage_key` 统一约束：
  - 存储含义：相对 `uploads` 的物理路径或对象 key，用于 resolver 映射磁盘路径。
...
2. 新上传写入：
   - 原始文件：`uploads/raw/<sha>.<ext>`
   - 衍生文件：`uploads/derived/<sha>.md`
...
3. 单条处理逻辑（幂等）：
   ...
   - 根据块类型与旧文件名生成 `storage_key`，并将旧扁平目录文件复制或硬链接到 `uploads/<storage_key>`；若暂不移动物理文件，则在 `attachment_files.legacy_source` 记录旧路径来源，供 resolver 从新 storage-key URL 回退到旧 `uploads` 扁平文件。
