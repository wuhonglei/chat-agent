...
- 本期迁移只覆盖聊天附件，不调整 workspace 物理目录：
  ...
  - **会话挂载迁移**：新增 `user_data/<user_id>/shared/uploads/manifest.json` 作为用户级上传挂载索引，以 `conversation_id` 为结构 key 记录各会话附件挂载关系；只作用于附件 uploads，不改变 `workspaces/<workspace_id>` 目录。
...
- **第二层：会话挂载迁移**。会话与附件的关系通过 manifest 表达，不引入新的路径协议；workspace 仍沿用旧 `workspaces/<workspace_id>` 目录。
...
- 写入策略：
  ...
  - 在 `shared/uploads/manifest.json` 的 `conversations.<conversation_id>.uploads` 下新增挂载记录
...
- 结合消息 `conversation_id` 建挂载关系，并保证历史文件可通过新 `storage_key` 访问：
  ...
  - 若暂不迁移物理文件，则 manifest 需记录 legacy source，resolver 对新 storage-key URL 可回退到旧 `uploads` 文件。
...
- 建议补充 `file_index` 与 `mount_index`。本期先用 `shared/uploads/manifest.json` 作为文件版临时索引，必须明确原子写、并发锁、损坏恢复与补偿重试；如果迁移需要强一致或跨实例部署，优先数据库化。
- 会话上传挂载索引固定在 `user_data/<user_id>/shared/uploads/manifest.json`；真实文件与索引同属 `shared/uploads`，manifest 只记录挂载关系，不存放文件内容。
- 预览接口安全策略：
...
- 会话挂载索引：
  - `user_data/<user_id>/shared/uploads/manifest.json`
  - 顶层按 `conversations.<conversation_id>` 组织该用户所有会话的挂载关系。
- Resolver 处理流程：
  1. 从 `url` 或显式 `storage_key` 得到相对存储路径，并由 `storage_key` 首段识别 `kind=raw|derived`。
  2. 读取 `shared/uploads/manifest.json`，在 `conversations.<conversation_id>.uploads` 中确认该 `storage_key` 已挂载到当前会话，并反查对应 `id`。
  3. 执行 `user_id + conversation_id + mount relation` 权限校验。
  4. 将 `storage_key` 映射到 `shared/uploads/<storage_key>`。
  ...
  "conversations": {
    "<conversation_id>": {
      "uploads": [
        {
          "id": "<sha256>",
          "storage_key": "raw/<sha256>.pdf",
          "kind": "raw",
          "mime": "application/pdf",
          "physical_path": "shared/uploads/raw/<sha256>.pdf",
          "display_name": "合同.pdf"
        },
        {
          "id": "<sha256>",
          "storage_key": "derived/<sha256>.md",
          "kind": "derived",
          "mime": "text/markdown",
          "derived_from_id": "<sha256>",
          "derived_kind": "pdf_to_markdown",
          "physical_path": "shared/uploads/derived/<sha256>.md",
          "display_name": "合同.md"
        }
      ]
    }
...
- 关系约束：
  ...
  - 同一 `id` 可对应多个 `storage_key`，但同一会话 manifest 中 `storage_key` 应唯一。
...
2. 新增 `resolve_file_ref(ref, user_id, conversation_id)`，支持旧 `url`、新 storage-key `url`、`id`、`storage_key` 输入。
...
2. 新上传写入：
   ...
   - 会话挂载：`shared/uploads/manifest.json` 的 `conversations.<conversation_id>.uploads`
3. 返回与落库：
...
3. 单条处理逻辑（幂等）：
   ...
   - 根据块类型与旧文件名生成 `storage_key`，并将旧文件复制或硬链接到 `shared/uploads/<storage_key>`；若暂不移动物理文件，则在 manifest 记录 legacy source，供 resolver 从新 storage-key URL 回退到旧 `uploads`。
   - 根据 `conversation_id` 生成/复用挂载记录，按 `(conversation_id, storage_key)` 幂等去重。
