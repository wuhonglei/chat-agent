...
- 将附件存储从当前 `user_data/<user_id>/uploads` 逐步迁移为：
  ...
  - `user_data/<user_id>/conversations/<conversation_id>/uploads/manifest.json`（会话上传挂载索引）
...
说明：
- `conversation/uploads` 实现为“虚拟视图（挂载关系）”，不做文件拷贝；挂载索引存放在 `conversations/<conversation_id>/uploads/manifest.json`。
- 真实文件位置只在服务端解析，Agent/前端只接触虚拟引用。
- Agent 可见的虚拟路径由 resolver 动态生成，例如：
  - `/conversations/<conversation_id>/uploads/raw/<file_id>`
  - `/conversations/<conversation_id>/uploads/derived/<file_id>`
...
- 物理存储路径：
  - `user_data/<user_id>/shared/uploads/raw/<file_id>`
  - `user_data/<user_id>/shared/uploads/derived/<file_id>`
- 会话挂载索引：
...
- Agent/前端可见虚拟路径：
  ...
  - `/conversations/<conversation_id>/uploads/derived/<file_id>`
- Resolver 处理流程：
  1. 解析虚拟路径得到 `conversation_id`、`kind=raw|derived`、`file_id`。
  2. 读取 `uploads/manifest.json`，确认该 `file_id` 已挂载到当前会话。
  3. 执行 `user_id + conversation_id + mount relation` 权限校验。
  4. 将虚拟路径映射到 `shared/uploads/raw|derived/<file_id>`。
...
```json
{
  "uploads": [
    {
      "file_id": "file_<sha256>.pdf",
      "kind": "raw",
      "physical_path": "shared/uploads/raw/file_<sha256>.pdf",
      "display_name": "合同.pdf"
    },
    {
      "file_id": "drv_<sha256>.md",
      "kind": "derived",
      "derived_from_file_id": "file_<sha256>.pdf",
...
```
- `vpath` 只作为动态响应字段或 Agent 工具入参使用，不写入消息持久化数据；消息持久化仍以 `file_id + storage_version` 为准。
