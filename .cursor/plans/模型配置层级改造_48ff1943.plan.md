...
models:
  ...
  scenarios:
    ...
    # compression / vision / coding / planning：可先写入配置，调用方后续再接
```

**Provider 命名建议**：按 API 端点聚合（如 `dashscope`、`deepseek`），`models` 下键为供应商 **API model_name**（与现有 `model_name` 字段一致），避免与展示名 `name` 混淆。

**capabilities → image_support**：`"vision" in capabilities` 等价于当前 `image_support: true`（供 [backend/app/api/chat.py](backend/app/api/chat.py) 图片校验与 [backend/app/api/models.py](backend/app/api/models.py) 列表）。
...
- `ProviderModelMeta`：`name`, `description`, `context_limit`, `capabilities: list[Literal["text","vision",...]]`
...
未接线的场景（`compression`、`vision`、`coding`、`planning`）：仅校验配置合法性，不在本次改调用链。
...
- 将 [backend/tests/test/config.py](backend/tests/test/config.py) 提升为正式 fixture：例如 `tests/fixtures/models_config.yaml` + `tests/test_model_resolver.py`，覆盖：
  - ref 解析、capabilities → image_support
  - scenario 解析
