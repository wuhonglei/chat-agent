...
todos:
  - id: schemas-models-config
    content: 在 app/schemas/config.py 定义 Provider/Scenario/ModelsConfig，删除 SummarizerModelConfig；Settings 用 models 替换 model_map/title_model/summarizer_model 并加校验
    status: pending
  - id: model-resolver
    content: 实现 app/services/base_service/model_resolver.py：resolve_ref、resolve_scenario、list_text_generation_models；单元测试 + fixtures YAML
    status: pending
  - id: wire-call-sites
    content: 替换 chat.py、models.py、chat_service.py、context_summary_service.py、chat schema 默认 model_id
    status: pending
  - id: token-context-limit
    content: ProviderModelMeta.context_limit 必填；LLMConfig 携带 context_limit；TokenCalculator 仅使用配置值，移除对 MODEL_LIMITS 的 chat 模型回退
    status: pending
...
保留 `LLMConfig` 作为 **运行时解析结果**，在现有字段基础上 **新增必填 `context_limit: int`**（由 resolver 从 `ProviderModelMeta` 写入）。`LLMService` / `BaseAgent` 构造签名可不变，但内部 `TokenCalculator` 必须使用 `llm_config.context_limit`。
...
| 方法 | 行为 |
|------|------|
| `resolve_ref(ref: str) -> LLMConfig` | 解析 `provider/model_key`：合并 provider 的 `base_url`/`api_key`/`options`，`model_name` = model_key，`title`/`description`/`context_limit` 来自 meta，`image_support` 来自 capabilities |
...
- [backend/app/utils/token.py](backend/app/utils/token.py)：`TokenCalculator` 构造增加必填参数 `context_limit: int`（或仅从 `LLMConfig` 构造）；`get_max_context_tokens()` **直接返回**该值。
- [backend/app/services/base_service/llm_service.py](backend/app/services/base_service/llm_service.py) 及所有用 `model_name` 单独 `new TokenCalculator(model_name)` 的聊天/Agent 路径：改为传入 `llm_config.context_limit`（经 resolver 解析后的配置）。
- 删除或收缩 `MODEL_LIMITS` 中对 **providers.models 已配置** 的 chat 模型条目，避免与 Nacos 双源；`embedding_model` 等未纳入 `models.providers` 的路径可暂保留 `MODEL_LIMITS`/`DEFAULT_LIMIT` 回退。
- Nacos 迁移时为每个 `models.*` 条目补齐 `context_limit`（可参考现有 `TokenCalculator.MODEL_LIMITS` 或厂商文档）。
