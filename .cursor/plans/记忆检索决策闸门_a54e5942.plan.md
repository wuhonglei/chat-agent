---
name: 记忆检索决策闸门
overview: 在现有 Mem0 搜索前加一层轻量 YES/NO 决策：短句规则先过滤，再用 flash 小模型判断当前问题是否需要个人记忆；不需要则跳过检索。失败一律放行检索，不阻断主对话。
todos:
  - id: config
    content: MemoryConfig 增加开关/超时/场景名；Nacos 增加可选 memory_retrieval 场景
    status: pending
  - id: classifier
    content: 实现 memory_retrieval_decision：flash 小模型 YES/NO、超时失败放行、Langfuse span
    status: pending
  - id: wire
    content: 在 ChatService._search_user_memories 中接入：短句 → 分类 → search
    status: pending
  - id: tests-docs
...
- **串行**：先分类再决定是否 search（目标是少打 Mem0，不是并行检索后丢弃）。
- **失败放行**：超时、解析失败、场景缺失 → 仍 search，不阻断 TTFT 之外的主链路。
- **可关闭**：`MemoryConfig` 开关，默认开启。
...
`memory_config` 可只靠代码默认值，不必强制改 YAML；需要时再加开关/超时覆盖。
...
只改 [`ChatService._search_user_memories`](backend/app/services/chat/chat_service.py)：

1. 空查询 / 短句确认 → 直接 `[]`（不调分类器）
2. `retrieval_decision_enabled` 为 false → 直接 search（现状）
3. 分类器返回不需要 → `[]`
4. 需要 / 失败放行 → 现有 `memory_service.search`

`ChatService.__init__` 里构造一次分类服务（或惰性创建）。Orchestrator 的预注入 memories / `task_action` 跳过逻辑保持不变。

### 4. 文档与测试

- 更新 [`backend/docs/用户管理.md`](backend/docs/用户管理.md)：闸门顺序、失败放行、配置项。
- 单测：
  - 解析 YES/NO/脏输出
  - 短句不调 LLM
  - NO → 不调 `MemoryService.search`
  - YES / timeout / 异常 → 仍 search
  - 开关关闭 → 不调分类器
