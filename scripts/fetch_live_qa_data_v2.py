...
def extract_tool_calls_from_content_blocks(content_blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """从 content_blocks 中提取工具调用信息"""
    tool_calls = []
...
def process_qa_pairs(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        # 从 content_blocks 中提取工具调用信息
        tool_calls = extract_tool_calls_from_content_blocks(row["assistant_content"] or [])
        ...
        server_names = [tc["server_name"] for tc in tool_calls if tc["server_name"]]
